...
> По-хорошему, конечно, в дальнейшем данный сервис будет разбит на множество микро-сервисов, но сейчас мне было удобнее объединить все в один.
...
В качестве формата данных для обмена информации внутри NATS все микро-сервисы используют JSON. Проще всего работать с NATS через его `nats.EncodedConn`, что позволит избежать всех "танцев"" с кодирование/декодированием данных при передаче.
...
Данный микро-сервис отвечает за вычисление приблизительных координат на основании данных о вышках сотовой связи.
