...
Для MongoDB крайне рекомендуется в URL так же указывать название базы данных.

В качестве формата данных для обмена информации внутри NATS все микросервисы используют формат JSON.
...
После разбора возвращает вычисленные координаты:
...
На входе получает координаты предполагаемой точки:
...
На входе данные для трекинга:

	{
		"Time": "2015-11-30T18:32:25.237+03:00",
		"Point": [55.765944,37.589248]
	}

Ответа никакого не генерится.
